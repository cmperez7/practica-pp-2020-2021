...
extern void solver_AU(int mode, int atoms_r, int atoms_l,  int nlig, float *rec_x, float *rec_y, float *rec_z, float *lig_x, float *lig_y, float *lig_z, int* rectype, int* ligtype, float *ql, float *qr, float *energy_desolv, struct autodock_param_t *a_params, int nconformaciones) 
{
	...
	switch (mode) {
		...
		case 1: //OpenMP execution
			...
			printf("**************************************\n");
      printf("\nATENCION: SI EJECUTAMOS UN PRAGMA, O DOS, Y SE QUEDA UNO O DOS SIN EJECUTAR, EN NUMERO DE HILOS PONEMOS 0\n\n");
      printf("Indique el numero de hilos a ejecutar en el primer pragma: \n");
			scanf("%d", &thread1);
      printf("Indique el numero de hilos a ejecutar en el segundo pragma: \n");
      scanf("%d", &thread2);
      printf("Indique el numero de hilos a ejecutar en el tercer pragma: \n");
      scanf("%d", &thread3);  
			printf("Conformations: %d\tMode: %d\n",nconformaciones,mode);			
			elapsed_i = wtime();
			forces_OMP_AU (atoms_r,atoms_l,nlig,rec_x,rec_y,rec_z,lig_x,lig_y,lig_z,rectype,ligtype,ql,qr,energy_desolv,a_params,nconformaciones, thread1, thread2, thread3);
			elapsed_o = wtime() - elapsed_i;
